...
type Classifier struct {
	...
	// Complexity classifier for complexity-based routing via NVIDIA service
	complexityClassifier *NvidiaComplexityClassifier
...
func NewClassifier(cfg *config.RouterConfig, categoryMapping *CategoryMapping, piiMapping *PIIMapping, jailbreakMapping *JailbreakMapping) (*Classifier, error) {
	...
	// Add NVIDIA complexity classifier if configured
	if cfg.ComplexityService != nil && cfg.ComplexityService.Enabled {
		ncc, err := NewNvidiaComplexityClassifier(cfg.ComplexityService)
		if err != nil {
			logging.Errorf("Failed to create NVIDIA complexity classifier: %v", err)
			return nil, err
		}
		options = append(options, withComplexityClassifier(ncc))
	}
...
func (c *Classifier) getAllSignalTypes() map[string]bool {
	...
	// Add complexity signals when NVIDIA service is enabled
	if c.Config.ComplexityService != nil && c.Config.ComplexityService.Enabled {
		allSignals[strings.ToLower(config.SignalTypeComplexity+":complexity")] = true
	}
...
func (c *Classifier) applySignalComposers(results *SignalResults) *SignalResults {
	// Note: NVIDIA complexity service does not use composer filters.
	// Complexity signals pass through unfiltered.
...
// filterComplexityByComposer is no longer needed — NVIDIA complexity service
// does not use composer conditions. Kept as a no-op for reference.
