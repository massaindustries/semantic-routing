...
func (s *ClassificationService) getUnmatchedSignals(signals *classification.SignalResults) *MatchedSignals {
	...
	// Check complexity rules (NVIDIA service produces a single "complexity" rule)
	if s.classifier.Config.ComplexityService != nil && s.classifier.Config.ComplexityService.Enabled {
		if !isMatched("complexity", signals.MatchedComplexityRules) {
			unmatched.Complexity = append(unmatched.Complexity, "complexity")
		}
	}
